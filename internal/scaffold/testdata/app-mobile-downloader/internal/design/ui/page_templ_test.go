...
func TestDesignPageAppPath(t *testing.T) {
	cases := []struct {
	...
	for _, c := range cases {
		if got := designPageAppPath(c.prefix, c.path); got != c.want {
			t.Errorf("designPageAppPath(%q, %q) = %q, want %q", c.prefix, c.path, got, c.want)
		}
	}
}

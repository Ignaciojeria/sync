...
// procRoot es el directorio usado como /proc por ListAgentRuntimes.
// Es una variable de paquete (no constante) únicamente para que los
// tests puedan apuntarla a un directorio temporal con procesos falsos
// sin depender de un Linux real bajo /proc.
var procRoot = "/proc"

// resolveCwd encapsula la lectura de /proc/<pid>/cwd. Separado del
// cuerpo de readRuntimeInfo para que los tests inyecten CWDs sin
// necesitar symlinks reales (que requieren privilegios elevados en
// Windows). En producción siempre vale os.Readlink.
var resolveCwd = func(proc string) (string, error) {
	return os.Readlink(filepath.Join(proc, "cwd"))
}
...
// ProcessInfo. Devuelve ok=false si no se pudo leer ni stat ni
// status — el proceso murió en medio del scan o nunca tuvo datos
// para nosotros.
func readRuntimeInfo(proc string, pid int) (ProcessInfo, bool) {
	r := ProcessInfo{PID: pid, Command: "pi"}

	if tgt, err := resolveCwd(proc); err == nil {
		r.CWD = tgt
	}

	hasData := false
	// /proc/<pid>/stat campo 22 = starttime en clock ticks desde boot.
	if statBytes, err := os.ReadFile(filepath.Join(proc, "stat")); err == nil {
		hasData = true
		fields := strings.Fields(string(statBytes))
		if len(fields) >= 22 {
			startJiffies, errP := strconv.ParseInt(fields[21], 10, 64)
			if errP == nil {
				uptimeBytes, errU := os.ReadFile(filepath.Join(procRoot, "uptime"))
				if errU == nil {

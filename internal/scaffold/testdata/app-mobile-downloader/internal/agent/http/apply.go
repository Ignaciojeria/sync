...
func applyHandler(s *server.Server, manager agentapp.AgentService, requireEditor func(http.Handler) http.Handler) {
	server.Handle(s, "POST /agent/sessions/{id}/apply", requireEditor(applyPreview(manager)))
}

func applyPreview(manager agentapp.AgentService) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		result, err := manager.ApplyPreview(r.Context(), id)
		if err != nil {
